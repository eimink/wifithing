...
static void set_led_rgb(uint32_t r, uint32_t g, uint32_t b)
{
    led_strip_set_pixel(led_strip, 0, r, g, b);
    led_strip_refresh(led_strip);
}

static void set_led_hsv(uint32_t h, uint32_t s, uint32_t v)
{
    uint32_t r, g, b;
...
static void color_wheel(uint32_t *hue)
{
    *hue = (*hue + 1) % 360;
    set_led_hsv(*hue, 100, 10);
}
...
void app_main(void)
{
    ...
    while (1) {
        color_wheel(&hue);
        vTaskDelay(10 / portTICK_PERIOD_MS); // Smaller delay for smoother transition
    }
